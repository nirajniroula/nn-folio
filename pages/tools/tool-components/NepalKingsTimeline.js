...
import { kingList } from "../../../constants/nepalKings";

const NepalKingsTimeline = ({ title }) => {
  ...
  useEffect(() => {
    ...
    // President Data
    const presidents = [...kingList]; // Create a copy of the array
    presidents.reverse();
    ...
    const scrollToPresident = (index) => {
      ...
      if (infoOverlay && presidentName && presidentTerm) {
        ...
        presidentTerm.textContent = `कार्यकाल: ${presidents[currentIndex].term}`;
    ...
    if (infoOverlay && presidentName && presidentTerm) {
      ...
      presidentTerm.textContent = `अवधि: ${presidents[0].term}`;
...
export default NepalKingsTimeline;
